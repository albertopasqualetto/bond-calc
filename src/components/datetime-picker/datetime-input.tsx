...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>((options: DateTimeInputProps, ref) => {
  ...
  const onFocus = useEventCallback(() => {
    ...
    setCurrentSegment(firstValidSegment);
    setSelection(inputRef, firstValidSegment);
  }, [validSegments]);
  ...
  return (
    ...
    >
      {!options.hideCalendarIcon && (
        <Button tabIndex="-1" variant="ghost" size="icon" onClick={options.onCalendarClick}>
          <CalendarIcon className="size-4 text-muted-foreground" />
        </Button>
      )}
      <input
        ref={mergeRefs(inputRef)}
        className="flex-grow min-w-0 bg-transparent py-1 pe-2 text-sm focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 selection:bg-primary selection:text-primary-foreground"
