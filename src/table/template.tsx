...
const getColumnId = <TData, TValue>(
...
const isPrintableColumn = <TData, TValue>(
	column: ColumnDef<TData, TValue>,
): boolean => {
...
const collectAllColumnIds = <TData, TValue>(
	columns: ColumnDef<TData, TValue>[],
): string[] => {
	const ids: string[] = [];
...
const collectHiddenPrintColumnIds = <TData, TValue>(
	columns: ColumnDef<TData, TValue>[],
): string[] => {
	const ids: string[] = [];
...
function DataTableInner<TData, TValue>({
	...
	const extractRowsFromPayload = useCallback(
	...
	useEffect(() => {
		const checkPrintMode = () => {
			...
			setColumnVisibility((prev) => ({
				...prev,
				...buildColumnVisibility(columns, isPrintMode),
			}));
		};
	...
	useEffect(() => {
		setColumnVisibility((prev) => ({
			...prev,
			...buildColumnVisibility(columns, isPrinting),
		}));
	...
	useEffect(() => {
		...
		localStorageKey,
		onDataChange,
		rewriteStoragePayload,
		shouldMigratePayload,
	]);
		...
		<div className="rounded-md border">
					...
					{onDeleteAllRows && (
						<Button
							...
							className={cn(
								...
								meta?.deleteConfirmState &&
									"bg-white-500 text-red-500 border",
							)}
						>
							<Trash2 />{" "}
							{meta?.deleteConfirmState
								? t("table.actions.confirmDeleteAll")
			...
			<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{t("table.dialogs.rowDetailsTitle")}
						</DialogTitle>
						<DialogDescription>
						...
						<div className="py-4">
							{renderRowDetails
								...
								: columns.map((column, i) => {
										const id = getColumnId(column);
										if (!id) return null;
										const selectedRowCandidate: unknown =
											selectedRow;
